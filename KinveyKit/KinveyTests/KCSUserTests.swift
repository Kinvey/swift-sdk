...
class KCSUserTests: KCSTestCase {
    ...
    func testCreateAutogeneratedUser() {
        ...
        KCSUser.requestCreateAutogeneratedUser(
            nil,
    ...
    func testCreateAutogeneratedUserCancel() {
        ...
        var executed = false
        let request = KCSUser.requestCreateAutogeneratedUser(
            nil,
            completion: { (user: KCSUser!, error: NSError!, actionResult: KCSUserActionResult) -> Void in
                executed = true
    ...
    func testRefreshUserCancel() {
        ...
        if let user = KCSUser.activeUser() {
            ...
            let request = user.requestRefreshFromServer { (result: [AnyObject]!, error: NSError!) -> Void in
                XCTAssertTrue(NSThread.isMainThread())
                XCTFail()
    ...
    func testChangePasswordCancel() {
        ...
        if let user = KCSUser.activeUser() {
            ...
            let request = user.requestChangePassword(
                "1234",
                completionBlock: { (results: [AnyObject]!, error: NSError!) -> Void in
                    XCTAssertTrue(NSThread.isMainThread())
    ...
    func testSaveUserCancel() {
        ...
        if let user = KCSUser.activeUser() {
            ...
            let request = user.requestSaveWithCompletionBlock { (results: [AnyObject]!, error: NSError!) -> Void in
                XCTAssertTrue(NSThread.isMainThread())
                XCTFail()
    ...
    func testDeleteUserCancel() {
        ...
        if let user = KCSUser.activeUser() {
            ...
            let request = user.requestRemoveWithCompletionBlock { (results: [AnyObject]!, error: NSError!) -> Void in
                XCTAssertTrue(NSThread.isMainThread())
                XCTFail()
