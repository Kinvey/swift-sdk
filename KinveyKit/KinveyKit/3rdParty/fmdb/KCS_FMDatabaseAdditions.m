...
- (uint32_t)applicationID {
    ...
    KCS_FMResultSet *rs = [self executeQuery:@"pragma application_id"];
...
- (void)setApplicationID:(uint32_t)appID {
    ...
    KCS_FMResultSet *rs = [self executeQuery:query];
    [rs next];
    [rs close];
}
...
- (uint32_t)userVersion {
    ...
    KCS_FMResultSet *rs = [self executeQuery:@"pragma user_version"];
