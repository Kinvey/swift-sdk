                                   ...
                                   completion:(KCSUser2CompletionBlock)completionBlock
{
    ...
    return [self requestCreateUserWithUsername:nil password:nil fieldsAndValues:fieldsAndValues completion:completionBlock];
}
...
{
    [self requestCreateUserWithUsername:username
                               password:password
                        fieldsAndValues:fieldsAndValues
                             completion:completionBlock];
}

+(KCSRequest *)requestCreateUserWithUsername:(NSString *)username
...
+ (void)connectWithAuthProvider:(KCSUserSocialIdentifyProvider)provider
               accessDictionary:(NSDictionary*)accessDictionary
                     completion:(KCSUser2CompletionBlock)completionBlock
{
    [self requestConnectWithAuthProvider:provider
                        accessDictionary:accessDictionary
                              completion:completionBlock];
}

+(KCSRequest *)requestConnectWithAuthProvider:(KCSUserSocialIdentifyProvider)provider
                             accessDictionary:(NSDictionary *)accessDictionary
                                   completion:(KCSUser2CompletionBlock)completionBlock
{
    SWITCH_TO_MAIN_THREAD_USER2_BLOCK(completionBlock);
                                                          ...
                                                          completion:^(KCSNetworkResponse *networkResponse, NSError *error)
    {
        if (error) {
            ...
            if (networkResponse.code == 200 && networkResponse.jsonData) {
                NSDictionary* jsonResponse = [NSJSONSerialization JSONObjectWithData:networkResponse.jsonData
                                                                             options:0
                                                                               error:&error];
                if (error) {
                    if (completionBlock) completionBlock(nil, error);
...
{
    [self requestChangePasswordForUser:user
                              password:newPassword
                            completion:completionBlock];
}

+(KCSRequest *)requestChangePasswordForUser:(id<KCSUser2>)user password:(NSString *)newPassword completion:(KCSUser2CompletionBlock)completionBlock
{
    SWITCH_TO_MAIN_THREAD_USER2_BLOCK(completionBlock);
    if (newPassword == nil) {
...
{
    [self requestRefreshUser:user
                     options:options
                  completion:completionBlock];
}

+(KCSRequest *)requestRefreshUser:(id<KCSUser2>)user options:(NSDictionary *)options completion:(KCSUser2CompletionBlock)completionBlock
{
    SWITCH_TO_MAIN_THREAD_USER2_BLOCK(completionBlock);
...
+(KCSRequest *)requestSaveUser:(id<KCSUser2>)user options:(NSDictionary *)options completion:(KCSUser2CompletionBlock)completionBlock
{
    SWITCH_TO_MAIN_THREAD_USER2_BLOCK(completionBlock);
...
{
    [self requestDeleteUser:user
                    options:options
                 completion:completionBlock];
}

+(KCSRequest *)requestDeleteUser:(id<KCSUser2>)user options:(NSDictionary *)options completion:(KCSCountBlock)completionBlock
{
...
{
    [self requestSendPasswordResetForUsername:username
                                   completion:completionBlock];
}

+(KCSRequest *)requestSendPasswordResetForUsername:(NSString *)username completion:(KCSUserSendEmailBlock)completionBlock
{
...
{
    [self requestSendPasswordResetForEmail:email
                                completion:completionBlock];
}

+(KCSRequest *)requestSendPasswordResetForEmail:(NSString *)email completion:(KCSUserSendEmailBlock)completionBlock
{
    SWITCH_TO_MAIN_THREAD_USER_SEND_EMAIL_BLOCK(completionBlock);
...
+(KCSRequest *)requestSendEmailConfirmationForUser:(NSString *)username completion:(KCSUserSendEmailBlock)completionBlock
{
    SWITCH_TO_MAIN_THREAD_USER_SEND_EMAIL_BLOCK(completionBlock);
...
{
    [self requestSendForgotUsernameEmail:email
                              completion:completionBlock];
}

+(KCSRequest *)requestSendForgotUsernameEmail:(NSString *)email completion:(KCSUserSendEmailBlock)completionBlock
{
    SWITCH_TO_MAIN_THREAD_USER_SEND_EMAIL_BLOCK(completionBlock);
    if (email == nil) {
        [[NSException exceptionWithName:NSInvalidArgumentException reason:@"email cannot be nil" userInfo:nil] raise];
...
+(KCSRequest *)requestCheckUsername:(NSString *)potentialUsername completion:(KCSUserCheckUsernameBlock)completionBlock
{
