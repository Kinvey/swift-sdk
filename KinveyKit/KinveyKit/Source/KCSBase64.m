...
//
static unsigned char KCSbase64EncodeLookup[65] =
...
//
static unsigned char KCSbase64DecodeLookup[256] =
{
                      ...
                      size_t *outputLength)
{
	...
	while (i < length)
	{
		...
		while (i < length)
		{
			unsigned char decode = KCSbase64DecodeLookup[inputBuffer[i++]];
			if (decode != xx)
			{
    ...
    * BASE64_UNIT_SIZE;
	...
	while (true)
	{
		...
		for (; i + BINARY_UNIT_SIZE - 1 < lineEnd; i += BINARY_UNIT_SIZE)
		{
			...
			//
			outputBuffer[j++] = KCSbase64EncodeLookup[(inputBuffer[i] & 0xFC) >> 2];
			outputBuffer[j++] = KCSbase64EncodeLookup[((inputBuffer[i] & 0x03) << 4)
                                                   | ((inputBuffer[i + 1] & 0xF0) >> 4)];
			outputBuffer[j++] = KCSbase64EncodeLookup[((inputBuffer[i + 1] & 0x0F) << 2)
                                                   | ((inputBuffer[i + 2] & 0xC0) >> 6)];
			outputBuffer[j++] = KCSbase64EncodeLookup[inputBuffer[i + 2] & 0x3F];
		}
...
NSData *KCSdataFromBase64String(NSString *aString)
{
	...
	size_t outputLength;
	void *outputBuffer = KCSNewBase64Decode([data bytes], [data length], &outputLength);
