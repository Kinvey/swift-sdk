...
internal class FindOperation<T: Persistable>: ReadOperation<T, [T]?> {
    ...
    init(query: Query, deltaSet: Bool, readPolicy: ReadPolicy, cache: Cache, client: Client, resultsHandler: ResultsHandler? = nil) {
        self.query = query
        ...
        request.execute() { data, response, error in
                ...
                let jsonArray = self.client.responseParser.parseArray(data)
            {
                ...
                if let cache = self.cache where deltaSet {
                    ...
                    if allIds.count > MaxIdsPerQuery {
                        ...
                        for offset in 0.stride(to: allIds.count, by: MaxIdsPerQuery) {
                            ...
                            let promise = Promise<[T]> { fulfill, reject in
                                ...
                                let operation = FindOperation(query: query, deltaSet: false, readPolicy: .ForceNetwork, cache: cache, client: self.client) { jsonArray in
                        ...
                        when(promises).thenInBackground { results in
                            ...
                            if deleted.count > 0 {
                                let query = Query(format: "\(T.idKey) IN %@", deleted)
                    ...
                    let persistableArray: [T] = T.fromJson(jsonArray)
