...
class DeltaSetCacheTestCase: KinveyTestCase {
    ...
    func test() {
        ...
        do {
            ...
            let store = DataStore<Book>.collection(.network)
            
            store.remove(byId: book.entityId!, options: nil) {
                switch $0 {
                case .success(let count):
                    XCTAssertEqual(count, 1)
                case .failure(let error):
        ...
        do {
            ...
            store.count(options: nil) { (result: Result<Int, Swift.Error>) in
                switch result {
                case .success(let _count):
                    XCTAssertEqual(Int64(_count), count + 1)
                case .failure(let error):
                    XCTFail(error.localizedDescription)
                }
