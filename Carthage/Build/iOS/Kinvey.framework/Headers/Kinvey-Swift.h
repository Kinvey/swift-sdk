...
*/
+ (BOOL)loginWithRedirectURI:(NSURL * _Nonnull)redirectURI micURL:(NSURL * _Nonnull)micURL client:(Client * _Nonnull)client;
/**
  Presents the MIC View Controller to sign in a user using MIC (Mobile Identity Connect).
*/
+ (void)presentMICViewControllerWithRedirectURI:(NSURL * _Nonnull)redirectURI timeout:(NSTimeInterval)timeout forceUIWebView:(BOOL)forceUIWebView client:(Client * _Nonnull)client completionHandler:(void (^ _Nullable)(User * _Nullable, NSError * _Nullable))completionHandler;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
@end
